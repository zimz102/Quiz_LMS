...
//   //     <div className="flex flex-col lg:flex-row lg:items-start gap-10 pr-20 pl-5 w-full">
//   //       {/* LEFT SIDE */}
...
//   //           onNavigate={(index) => {
//   //             setCurrentQuestionIndex(index); // Cập nhật lại chỉ số câu hiện tại
...
//                           {submitted && (
//                             <p
//                               className={`mt-2 font-semibold ${(answers[questionKey] || "").trim().toLowerCase() === item.answer.trim().toLowerCase()
//                                 ? "text-green-600"
//                                 : "text-red-600"
//                                 }`}
//                             >
...
//               const el = document.querySelector(`[data-question-id="${multipleChoiceQuestions[index].id}"]`);
//               console.log(`Navigating to question ID: ${multipleChoiceQuestions[index].id}`);
//               if (el) {
//                 el.scrollIntoView({ behavior: "smooth", block: "start" });
//               }
//             }}
...
import React, { useState } from 'react';
import Login from './Login';
import MainApp from './MainApp'; // import component đã tách
...
export default function App() {
  ...
  return <MainApp user={user} setUser={setUser} />; // Truyền xuống để logout
}
